    ...
    public function getModules()
    {
        ...
        if(!isset($modules[$appKey])) {
            ...
            if(!($moduleDirs = cache::getCached($cacheKey))) {
                ...
                $applicationModulesDir = $this->getApplicationDirectory() . '/modules';
                $searchModulesDir = file_exists($applicationModulesDir) ? scandir($applicationModulesDir) : [];
                $searchModulesDir = array_merge($searchModulesDir, $this->getBundlesModulesDirs());

                foreach ($searchModulesDir as $moduleName) {
                    if ($moduleName[0] === '.') {
                        continue;
                    }
    ...
    public function dispatch(action $action, array $arguments = [], $withoutBase = null, $stopApp = false)
    {
        ...
        if($isApiRequest) {
            ...
            if (($timeNow - $user->getLastActivity() > self::REFRESH_SESSION_AFTER) && !$user->isGuest()) {
                ...
                $user->getMapper()->save($user, modelBase::UPDATE__MODE_CHANGES_ONLY);
